...
export const PageTranslationProvider: React.FC<PageTranslationProviderProps> = ({
  ...
  // ALWAYS start with translation DISABLED
  // Translation only happens when user clicks the button
  ...
  const enableTranslation = useCallback(() => {
    setIsEnabled(true);
    setError(null);
    // Don't persist to localStorage - only translate on explicit click
  }, []);
